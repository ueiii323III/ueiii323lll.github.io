...
import { ExerciseCalorieCalculator } from '../../utils/health-calculator'
import { ExerciseRecord } from '../../typings/types'

Page({
  data: {
    todayRecords: [] as ExerciseRecord[],
    totalDuration: 0,
    totalCalories: 0,
    targetDuration: 0,
    durationProgress: 0,
    ...
    // 删除相关
    recordToDelete: null as ExerciseRecord | null,
  ...
  initPage() {
    ...
    // 获取运动类型列表
    const exerciseTypes = ExerciseCalorieCalculator.getExerciseList()
    this.setData({
      exerciseTypes
    })
  ...
  calculateCalories() {
    ...
    if (exerciseType && duration && duration > 0) {
      const calories = ExerciseCalorieCalculator.calculateCalories(
        exerciseType,
        duration,
        this.data.intensity as any,
        userWeight
      )
  ...
  addExerciseRecord() {
    ...
    const calculatedCalories = ExerciseCalorieCalculator.calculateCalories(
      exerciseType, 
      duration, 
      this.data.intensity as any, 
      weight
    ...
    const record: ExerciseRecord = {
      ...
      exerciseType,
      duration,
      intensity: this.data.intensity as any,
    ...
    StorageHelper.saveTodayRecords(todayRecords)
  ...
  // 获取强度显示文本
  getIntensityText(intensity: string): string {
    const intensityMap = {
      'low': '低强度',
      'medium': '中强度',
      'high': '高强度'
    }
    return intensityMap[intensity as keyof typeof intensityMap] || '未知'
  },
