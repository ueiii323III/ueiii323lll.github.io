...
import { FoodCalorieCalculator } from '../../utils/health-calculator'
import { FoodRecord } from '../../typings/types'

Page({
  data: {
    todayRecords: [] as FoodRecord[],
    totalCalories: 0,
    targetMinCalories: 0,
    targetMaxCalories: 0,
    progress: 0,
    ...
    // 删除相关
    recordToDelete: null as FoodRecord | null,
  ...
  loadTodayRecords() {
    ...
    // 按时间排序（最新的在前）
    const sortedRecords = foodRecords.sort((a, b) => 
  ...
  onFoodNameInput(e: any) {
    ...
    // 获取食物建议
    if (value.length > 0) {
      const suggestions = FoodCalorieCalculator.getFoodSuggestion(value)
      this.setData({
  ...
  calculateCalories() {
    ...
    if (foodName && amount && amount > 0) {
      const calories = FoodCalorieCalculator.calculateCalories(
        foodName,
        amount,
        this.data.unit
      )
  ...
  addFoodRecord() {
    ...
    const record: FoodRecord = {
      ...
      mealType: this.data.mealType as any,
      foodName,
      amount,
      unit: this.data.unit,
      calories: this.data.calculatedCalories || 
                FoodCalorieCalculator.calculateCalories(foodName, amount, this.data.unit),
  ...
  getMealTypeText(mealType: string): string {
    ...
    return typeMap[mealType as keyof typeof typeMap] || '其他'
  },
