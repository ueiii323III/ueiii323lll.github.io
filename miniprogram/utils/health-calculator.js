...
exports.HealthAdviceGenerator = exports.ExerciseCalorieCalculator = exports.FoodCalorieCalculator = exports.HealthGoalsCalculator = exports.TargetWeightCalculator = exports.BMICalculator = void 0;
// BMI计算器
class BMICalculator {
    ...
    static getBMICategoryText(category) {
        const categoryMap = {
            'underweight': '偏瘦',
            'normal': '正常',
            'overweight': '超重',
...
class TargetWeightCalculator {
    static calculateTargetWeight(profile) {
        const { height, gender, age, healthGoal, activityLevel, weight } = profile;
        
        // 如果没有体重数据，使用默认值来计算目标体重范围
        const weightForCalc = weight || height * 0.22; // 简单估算：身高(cm) × 0.22 ≈ 理想体重
        
        // 基础代谢率计算
        let bmr;
        if (gender === 'male') {
            bmr = 88.362 + (13.397 * weightForCalc) + (4.799 * height) - (5.677 * age);
        }
        else {
            bmr = 447.593 + (9.247 * weightForCalc) + (3.098 * height) - (4.330 * age);
        }
        // 活动系数
...
exports.TargetWeightCalculator = TargetWeightCalculator;
// 健康目标计算器
class HealthGoalsCalculator {
...
exports.HealthGoalsCalculator = HealthGoalsCalculator;
// 食物热量计算器
class FoodCalorieCalculator {
    static calculateCalories(foodName, amount, unit) {
    ...
    static getFoodSuggestion(keyword) {
        const suggestions = [];
...
exports.FoodCalorieCalculator = FoodCalorieCalculator;
FoodCalorieCalculator.foodDatabase = {
...
class ExerciseCalorieCalculator {
    static calculateCalories(exerciseType, duration, intensity, weight) {
        // 验证输入参数
...
exports.ExerciseCalorieCalculator = ExerciseCalorieCalculator;
ExerciseCalorieCalculator.exerciseMET = {
    '跑步': 8.0,
    '游泳': 7.0,
    '骑行': 6.0,
    '健身': 5.0,
...
class HealthAdviceGenerator {
    static generateRealtimeAdvice(waterRecords, foodRecords, exerciseRecords, healthGoals, userProfile) {
        const advices = [];
        // 饮水建议
    ...
    static generatePeriodicAdvice(weeklyData, userProfile) {
        const advices = [];
        // 分析一周的运动情况 - 修复数据结构访问
        const exerciseDays = weeklyData.filter(day => day.records && day.records.exerciseRecords && day.records.exerciseRecords.length > 0).length;
        if (exerciseDays < 3) {
